...
#你的selenium的驱动地址：
driverUrl='/Users/xxx/Downloads/chromedriver'
#driverUrl='D:\pypackages\chromedriver.exe'

#你的pubmed网址（先按规则搜索，copy第一个结果页面到这里）：
webUrl='https://pubmed.ncbi.nlm.nih.gov/?term=nervous+system'
...
def items_per_page():
    ...
    time.sleep(1)
    driver.find_element(By.CLASS_NAME, "trigger").click()
    tags_List = driver.find_element(By.ID, "id_size").find_elements(By.TAG_NAME, "option")
    ...
    for tag_List in tags_List:
        ...
        i+=1    
    time.sleep(0.3)
    print('已将pubmed搜索结果的每页显示20项改成200项，减少翻页次数')
...
def PMID_to_save():
    #读取信息
    PMID=[]
    articles=driver.find_elements(By.CLASS_NAME, "docsum-content") #定位到200篇文章信息的外层位置
    for article in articles:
        PMID.append(article.find_element(By.CLASS_NAME, "docsum-pmid").get_attribute("textContent")) #获取文章的PMID
    #逐行储存信息
    PMID_index =open("/Users/yidong1/Downloads/PMID.csv",'a',newline='')
    for pmid in PMID:
...
def page_turning(page_target):
    driver.find_element(By.ID, "page-number-input").clear()
    time.sleep(0.3)
    driver.find_element(By.ID, "page-number-input").send_keys(page_target) #输入页码
    time.sleep(0.3)
    driver.find_element(By.ID, "page-number-input").send_keys(Keys.ENTER) #按回车键
...
def article_to_save():
    try:
        #获取PMID
        PMID=driver.find_element(By.CLASS_NAME, "current-id").get_attribute("textContent")
...
#        print(PMID)
    ...
    #获取期刊来源
    try:
        journal=driver.find_element(By.CLASS_NAME, "linkout-category-links").find_element(By.TAG_NAME, "a").get_attribute("textContent")
...
#    print(abstract)
    ...
    try:
        authors=driver.find_element(By.CLASS_NAME, "authors-list").find_elements(By.CLASS_NAME, "full-name")
        authorList=[]
...
#    print(authInfo)
    ...
    #储存获取的信息
    MyPubmedData=open("/Users/yidong1/Downloads/MyPubmedData.csv",'a',newline='',encoding='utf-8')
...
def PMID_url_err_save(PMID_url):
    PmidUrlErr =open("/Users/yidong1/Downloads/PmidUrlErr.csv",'a',newline='',encoding='utf-8')
...
my_user='xxx@163.com' #收件人邮箱账号
def email_progress(myMessage):
    try:
        msg=MIMEText(myMessage,"plain","utf-8")
        msg['From']=formataddr(['尊敬的xxx',my_sender]) #括号里的对应发件人邮箱昵称（str）、发件人邮箱账号
        msg['To']=formataddr(['Hong Yikai',my_user]) #括号里的对应收件人邮箱昵称（str）、收件人邮箱账号
        msg['Subject']="文献下载情况汇报" #邮件的主题，也可以说是标题
        server=smtplib.SMTP("smtp.163.com",25) #发件人邮箱中的SMTP服务器，端口是25,SSL加密则根据各网站服务器定义的写
...
#================================================================================
if startPage != 0:
    s = Service(r"/Users/xxx/Downloads/chromedriver")
    driver = webdriver.Chrome(service=s)
    driver.get(webUrl)
    time.sleep(5)
    items_per_page()
    time.sleep(5)
    pages=driver.find_element(By.CLASS_NAME, "of-total-pages").get_attribute("textContent") #得到总页数
    pages=pages.replace(r"of ", '') 
    pages=pages.replace(r",", '') # 得到数字字符串
    pages=int(pages)
...
allPMIDs=0 
PMID_file =open("/Users/xxx/Downloads/PMID.csv",'r')
